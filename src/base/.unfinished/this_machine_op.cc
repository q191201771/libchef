...
namespace inner {
  ...
  class meminfo {
    public:
      ...
      int32_t mem_total_kb_;
      int32_t mem_free_kb_;
      int32_t mem_buffers_kb_;
      int32_t mem_cached_kb_;
      int32_t mem_used_kb_;
...
namespace chef {
  ...
  int32_t this_machine_op::num_of_available_cpu_cores() {
    return ::get_nprocs();
  }

  int32_t this_machine_op::mem_total_kb() {
    inner::meminfo mi;
    mi.parse();
    return mi.mem_total_kb_;
  }

  int32_t this_machine_op::mem_free_kb() {
    inner::meminfo mi;
    mi.parse();
    return mi.mem_free_kb_;
  }

  int32_t this_machine_op::mem_buffers_kb() {
    inner::meminfo mi;
    mi.parse();
    return mi.mem_buffers_kb_;
  }

  int32_t this_machine_op::mem_cached_kb() {
    inner::meminfo mi;
    mi.parse();
    return mi.mem_cached_kb_;
  }

  int32_t this_machine_op::mem_used_kb() {
    inner::meminfo mi;
    mi.parse();
    return mi.mem_used_kb_;
  }

  int64_t this_machine_op::boot_timestamp() {
    inner::stat st;
    st.parse();
    return st.btime_;
  }

  int64_t this_machine_op::up_duration_seconds() {
    inner::stat st;
    if (st.parse() == -1) {
      return 0;
    }
