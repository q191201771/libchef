...
namespace chef {
                                 ...
                                 COMPRESSION_LEVEL level)
  {
    ...
    unsigned char *out_data = (unsigned char *)malloc(out_data_capacity);
    ...
    do {
      ...
      strm.next_in = (unsigned char *)in + used;
      used += strm.avail_in;
      left -= strm.avail_in;
      ...
      do {
        ...
        if (out_data_len + have > out_data_capacity) {
          ...
          out_data_capacity <<= 1;
          unsigned char *tmp = (unsigned char *)realloc(out_data, out_data_capacity);
          if (tmp == NULL) {
            goto cleanup;
          }
          out_data = tmp;
