...
namespace chef {
  ...
  class buffer {
    public:
      /**
       * @param
       * init_capacity   初始化大小，后续不够时内部会自动扩容
       * shrink_capacity 收缩阀值，当申请内存大小大于shrink_capacity且实际使用小于
       *                 init_capacity时会释发多余内存，恢复成init_capacity
       */
      ...
      /**
       * 方法二直接操作write pos指针,写入前需确保内部空余空间够,且操作完后需手动移动write
       * pos位置.提供方法二是为了便于与其他字符串函数配合,在某些场景下减少一次内存拷贝.例如:
       * buf.reserve(128);
       ...
       * @function reserve
       * @param    len
       * @brief    确保buffer中空余空间大小>=[len],够时不做任何操作，不够则在内部扩容
       *
       * @function write_pos 写入位置
       *
